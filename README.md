...
The CIS department's `610` courses follow three major "units" common to all students, regardless of major. These are:

* Advising (advanced project research, artifact development)
* Reporting (thesis document writing)
* Presenting (final presnetation)

These units guide the tasks that we will complete through the semester. Work completed during
each unit will further your progress toward the global assignment due at the end of each unit.

#### Major-specific outcomes

This syllabus governs the following `610`-level courses in the Allegheny College Department of Computer and Information Science. While we meet weekly as one large group, each of us will pursue different course outcomes from our work. See the appropriate course below to review the learning outcomes that this course provides.
...
Continuation of independent research in computer science culminating in the development of and release of a computational artifact.
...
Continuation of independent research in data science culminating in the development of and release of a computational artifact. 
...
Independent research in informatics culminating in the development of a prototype or early draft of a computational artifact.
...
Continuation of independent research in software engineering culminating in the development of and release of a computational artifact.
...
The evaluative criteria in this course do not weight these equally, and place emphasis on product and presentation; however,
this does not make the process behind your work inconsequential. By nature, a good process necessitates a good product. There
...
Pay close attention to the above paragraph. Your `600` class is responsible for having created the foundation for the product(s)
that you will necessarily create this semester. This semester is intended to test the soundness of the foundational skills,
development, and research that you alredy conducted.
...
|:-----------|:---------|:------------------|
|Changelogs and meeting notes| `Process`   | 15%       |
|Class activities | `Process`   | 10%       |
|Computational artifact |`Product`| 25%     |
|Final presentation | `Product` | 25%       |
|Thesis writing     | `Product` | 25%       |

Failure to complete any of the items in the `Product` category identified above will result in a failing grade for this course.

##### Changelogs and meeting notes

###### Changelogs

For the Spring 2025 semester, `Research Journal` posts will be replaced by `Changelog`s.

The [Keep a Changelog](https://keepachangelog.com/) project provides some helpful contextualization around changelogs:
...
For the second semester of your Senior Project, you will be required to keep a changelog for _both_ your writing and computational artifact.
Begin numbering versions from the current number released as of class on `17 January 2025`. If time allows, you can and _should_ go back and
capture previous versions. Each post should contain the writing and software versions addressed as a title, e.g. `Thesis 0.8.1; <PROJECT TITLE> 0.9.5`.

These posts should categorize changes made to `thesis writing` and your `computational artifact`. Faculty expect that there are at least `5` changes for
each deliverable in each post, divided by headings. Each section should contain a short overview that characterizes the changes made and
the reason(s) for making them.
...
These activities will produce or revise deliverables required for the `Product` category of course assignments. Activities range
...
During the `610` semester course, students will complete sections `3`, `4`, and `5` for review by their reader(s). In addition,
students will refine and revise sections `1` and `2` in order to develop a compelling, accurate, and comprehensive thesis document.
In addition, students are required to release new minor versions of their thesis document on all due dates requiring drafts or 
revisions.
...
* overview
* value
* efficacy and effectiveness through experimental results
* future direction
...
a project and for allowing others in department's research community to evalute the overall soundness of a project's execution,
purpose, and implications. Often, if a researcher can't clearly articulate what their project _is_ and _does_, it suggests that
their research process lacks development and understanding of core research goals.

These presentations will last `10` minutes over the course of `3` weeks on `11 April`, `18 April`, and `25 April`. Guidelines for 
the presentations will be distributed after Spring Break.
